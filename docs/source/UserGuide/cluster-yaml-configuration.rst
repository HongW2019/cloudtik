...
Modifying the ``node_config`` and updating with ``cloudtik start CLUSTER_CONFIG_FILE`` will cause the cloudtik to scale down all existing nodes of the node type;
nodes with the newly applied ``node_config`` will then be created according to cluster configuration and cloudtik resource demands.
...
The number of minutes that need to pass before an idle worker node is removed by the cloudtik.
...
Authentication credentials that cloudtik will use to launch nodes.
...
Tells the cloudtik the allowed node types and the resources they provide.
...
If the field ``head_node_type`` is changed and an update is executed with ``cloudtik start CLUSTER_CONFIG_FILE``, the currently running head node will
...
.. tabbed:: AWS
    ...
    .. code-block:: yaml
        ...
        setup_commands:
          ...
          - pip install -U https://d30257nes7d4fq.cloudfront.net/downloads/cloudtik/cloudtik-0.9.0-cp37-cp37m-manylinux2014_x86_64.whl

- Setup commands should ideally be *idempotent* (i.e., can be run multiple times without changing the result); this allows cloudtik to safely update nodes after they have been created. You can usually make commands idempotent with small modifications, e.g. ``git clone foo`` can be rewritten as ``test -e foo || git clone foo`` which checks if the repo is already cloned first.

- Setup commands are run sequentially but separately. For example, if you are using anaconda, you need to run ``conda activate env && pip install -U cloudtik`` because splitting the command into two setup commands will not work.
...
If enabled, cloudtik will not automatically specify the size ``/dev/shm`` for the started container and the runtime's default value (64MiB for Docker) will be used.
If ``--shm-size=<>`` is manually added to ``run_options``, this is *automatically* set to ``True``, meaning that cloudtik will defer to the user-provided value.
...
The user that cloudtik will authenticate with when launching new nodes.
...
.. tabbed:: AWS
    ...
    The path to an existing private key for cloudtik to use. If not configured, cloudtik will create a new private keypair (default behavior). If configured, the key must be added to the project-wide metadata and ``KeyName`` has to be defined in the :ref:`node configuration <cluster-configuration-node-config>`.
...
.. tabbed:: Azure
    ...
    The path to an existing private key for cloudtik to use.
...
.. tabbed:: Azure
    ...
    The path to an existing public key for cloudtik to use.
...
.. tabbed:: AWS
    ...
    The region to use for deployment of the cloudtik cluster.
...
.. tabbed:: Azure
    ...
    The location to use for deployment of the cloudtik cluster.
...
.. tabbed:: Azure
    ...
    The resource group to use for deployment of the cloudtik cluster.

    * **Required:** Yes
    * **Importance:** High
    * **Type:** String
    * **Default:** cloudtik-cluster
...
.. tabbed:: Azure
    ...
    The subscription ID to use for deployment of the cloudtik cluster. If not specified, cloudtik will use the default from the Azure CLI.
...
.. tabbed:: GCP
    ...
    The globally unique project ID to use for deployment of the cloudtik cluster.
...
* **Importance:** Low
* **Type:** String
* **Default:** ``"cloudtik-workspace-{cluster-name}"``
...
The resources that a node type provides, which enables the cloudtik to automatically select the right type of nodes to launch given the resource demands of the application. The resources specified will be automatically passed to the ``cloudtik start`` command for the node via an environment variable. If not provided, cloudtik can automatically detect them only for AWS/Kubernetes cloud providers.
...
.. tabbed:: AWS
    ...
    The number of CPUs made available by this node. If not configured, cloudtik can automatically detect them only for AWS/Kubernetes cloud providers.
...
.. tabbed:: AWS
    ...
    The number of GPUs made available by this node. If not configured, cloudtik can automatically detect them only for AWS/Kubernetes cloud providers.
...
.. tabbed:: AWS
    ...
    The memory in bytes allocated for python worker heap memory on the node. If not configured, cloudtik will automatically detect the amount of RAM on the node for AWS/Kubernetes and allocate 70% of it for the heap.
...
.. tabbed:: AWS
    ...
    The memory in bytes allocated for the object store on the node. If not configured, cloudtik will automatically detect the amount of RAM on the node for AWS/Kubernetes and allocate 30% of it for the object store.
...
.. tabbed:: AWS
    ...
    .. literalinclude:: ../../../example/cluster/aws/example-standard.yaml
        :language: yaml

.. tabbed:: Azure

    .. literalinclude:: ../../../example/cluster/azure/example-standard.yaml
        :language: yaml

.. tabbed:: GCP

    .. literalinclude:: ../../../example/cluster/gcp/example-standard.yaml
        :language: yaml
